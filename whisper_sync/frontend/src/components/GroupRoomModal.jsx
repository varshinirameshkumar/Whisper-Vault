...
/** Multi-user debounced search — Santhya's Discovery Component */
function MultiUserSearch({ selectedUsers, onAdd, onRemove, currentUsername }) {
    ...
    <div ref={wrapperRef} style={{ position: 'relative' }}>
      ...
      {open && results.length > 0 && (
        <div className="scale-in" style={{
          ...
          borderRadius: 'var(--radius)', zIndex: 300, maxHeight: 200, overflowY: 'auto',
...
/**
 * GroupRoomModal — Unified Discovery & Handshaking
 * Lets the user select targets then choose: Secret Message OR Secret Chat Room
 */
export default function GroupRoomModal({ currentUsername, onClose, onRoomCreated, onSendSecret }) {
  const [selected, setSelected]   = useState([])
  const [mode, setMode]           = useState(null) // null | 'secret' | 'room'
  const [loading, setLoading]     = useState(false)
  ...
  const handleAction = async (chosenMode) => {
    ...
    try {
      if (chosenMode === 'secret') {
        onSendSecret(selected[0])
        onClose()
      } else {
        const room = await roomService.createRoom(selected.map(u => u.username))
        toast.success(`Vault room created! Invites sent to ${selected.length} participant(s).`)
        onRoomCreated(room)
        onClose()
      }
    } catch (err) {
  ...
  return (
    <div style={{
      ...
      <div className="fade-in vv-card" style={{ width: '100%', maxWidth: 480, padding: 28 }}>
        {/* Header */}
